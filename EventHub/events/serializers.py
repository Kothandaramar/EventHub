...
from .models import Event , Reservation ,Waitlist
...
class ReservationSerializer(serializers.ModelSerializer) : 
  ...
  def validate_seats_reserved(self , value) : 
    if value < 1 : 
      raise serializers.ValidationError('Must reserve atleast 1 seat')
    return value
  ...
  def validate(self, data):
    ...
    if data.get('seats_reserved', 0) > event.available_seats:
      raise serializers.ValidationError(f"Only {event.available_seats} seat(s) available. Option to join waitlist.")
    return data
    
  def create(self, validated_data):
    event = validated_data['event']
    if validated_data['seats_reserved'] > event.available_seats:
        return self.handle_waitlist(
            event,
            validated_data.get('attendee_name'),
            validated_data.get('attendee_email'),
            validated_data.get('seats_reserved')
        )
    event.available_seats -= validated_data['seats_reserved']
    event.save()
    return Reservation.objects.create(**validated_data)
  
  def handle_waitlist(self, event, attendee_name, attendee_email, seats_requested):
...
class WaitlistSerializer(serializers.ModelSerializer):
  ...
  class Meta:
    model = Waitlist
    fields = ['id', 'event', 'attendee_name', 'attendee_email', 'seats_requested', 'created_at']
